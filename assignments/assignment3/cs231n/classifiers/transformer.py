...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        features_proj = features_proj.unsqueeze(1).expand(-1, T, -1)  # [N, W] -> [N, 1, W] -> [N, T, W]
        ...
        output = self.transformer(captions_embed, features_proj, tgt_mask=tgt_mask)  # [N, T, W]

        # Project the output to scores per token.
        scores = self.output(output)  # [N, T, W] -> [N, T, V]
