...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = np.matmul(dnext_tanh, Wx.T)
    dprev_h = np.matmul(dnext_tanh, Wh.T)
    dWx = np.matmul(x.T, dnext_tanh)
    dWh = np.matmul(prev_h.T, dnext_tanh)
...
def rnn_backward(dh, cache):
    ...
    N, T, D = dh.shape
    dxc, dprev_h, dWx, dWh, db = rnn_step_backward(dh[:, T - 1, :], cache[T - 1])
    D = dxc.shape[1]
    dx = np.zeros((N, T, D))
    dx[:, T - 1, :] = dxc
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    - Wh: Hidden-to-hidden weightk-, of shape (H, 4H)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    dxc, dprev_h, dprev_c, dWx, dWh, db = lstm_step_backward(dh[:, T - 1, :], np.zeros((N, H)), cache[T - 1])
    D = dxc.shape[1]
    dx = np.zeros((N, T, D))
    dx[:, T - 1, :] = dxc
    for i in range(T - 2, -1, -1):
        dxc, dprev_hc, dprev_cc, dWxc, dWhc, dbc = lstm_step_backward(dh[:, i, :] + dprev_h, dprev_c, cache[i])
        dx[:, i, :] = dxc
        dprev_h = dprev_hc
        dprev_c = dprev_cc
        dWx += dWxc
