...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.n_head
        E_H = self.head_dim
        Q = self.query(query).view(N, S, H, E_H).transpose(1, 2)  # (N, H, S, E_H)
        K = self.key(key).view(N, T, H, E_H).permute(0, 2, 3, 1)  # (N, H, E_H, T)
        V = self.value(value).view(N, T, H, E_H).transpose(1, 2)  # (N, H, T, E_H)
        QK = torch.matmul(Q, K) / math.sqrt(E_H)  # (N, H, S, T)
        if attn_mask is not None:
            QK = QK.masked_fill(attn_mask == 0, float('-inf'))
        QK = F.softmax(QK, dim=-1)
        QK = self.attn_drop(QK)
        QKV = torch.matmul(QK, V).transpose(1, 2).contiguous().view(N, S, E)
