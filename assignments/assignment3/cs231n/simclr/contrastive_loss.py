...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_pos = torch.exp(sim(z_k, z_k_N) / tau)

        denom_k, denom_k_N = 0, 0
        ...
        loss_k = -torch.log(sim_pos / denom_k)
        loss_k_N = -torch.log(sim_pos / denom_k_N)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.matmul(out, out.T) / (torch.linalg.norm(out, dim=1).view(-1, 1) * torch.linalg.norm(out, dim=1).view(1, -1))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # Option 1
    sim_pos = torch.cat([sim_matrix[torch.arange(N), torch.arange(N, 2 * N, 1)], sim_matrix[torch.arange(N, 2 * N, 1), torch.arange(N)]], dim=0).view(-1, 1)  # [2*N, 1]

    # Option 2
    # sim_pos = torch.cat([sim_positive_pairs(out_left, out_right), sim_positive_pairs(out_right, out_left)], dim=0)
