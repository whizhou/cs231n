...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_labels = torch.ones(logits_real.squeeze().size()).type(dtype)
    fake_labels = torch.zeros(logits_fake.squeeze().size()).type(dtype)
...
def generator_loss(logits_fake):
    ...
    fake_labels = torch.ones(logits_fake.squeeze().size()).type(dtype)
